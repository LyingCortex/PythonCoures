...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        return min([p1.distance(p2)  for p1 in self.points for p2 in other.points])
    ...
    def averageLinkageDist(self, other):
        ...
        for p1 in self.points:
            for p2 in other.points:
                sum_dist += p1.distance(p2)
        return sum_dist / (len(self.points) * len(other.points))
    def members(self):
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        points = c1.points
        for p in c2.points:
            if p not in c1.points:
                points.append(p)
        a = Cluster(points, c1.pointType)
    ...
    def findClosest(self, linkage):
        ...
        for i in xrange(len(self.members)):
            for j in xrange(len(self.members)):
                if i != j :
                    a = linkage(self.members[i], self.members[j])
                    if metric > a:
                        metric = a
                        x = i
                        y = j
        return (self.members[x], self.members[y])
    def mergeOne(self, linkage):
        ...
        # TO DO
        c= self.findClosest(linkage)
        self.mergeClusters(c[0], c[1])
        return c

...
#
#
#nodes = {node for node in list(mitMap.nodes)}
#
##print mitMap.edges
#edgess = {}
#for k in mitMap.edges:
#    ap = []
#    for d in mitMap.edges[k]:
#        ap.append([d[0], (d[1], d[2])])
#    edgess[k] = ap
#print edgess
...
def directedDFS(digraph, start, end, maxTotalDist, maxDistOutdoors):
    ...
    Returns:
        ...
        maxDistOutdoors constraints, then raises a ValueError.
    """
    def dfs(graph, start_node, end_node, max_total, max_outdoor, path=[], valid_paths=[], total=0, outdoor=0):
        path = path + [start_node]
        if len(path) >= 2:
            edge = lookup_edge(graph, path[-2], path[-1])
            total += float(edge[1])
            outdoor +=float(edge[2])
            if total > max_total or max_outdoor > max_outdoor:
                return valid_paths
        if start_node == end_node:
            valid_paths.append(path)
            return valid_paths
        for node in graph.childrenOf(start_node):
            if node not in path:
                valid_paths = dfs(graph, node, end_node, max_total, max_outdoor, path, valid_paths, total, outdoor)
        return valid_paths
    ...
    def lookup_edge(graph, source, destination):
        ...
        for edge in edges:
            if edge[0] == destination:
                return edge
        raise ValueError
    valid_paths = dfs(digraph, Node(start), Node(end), maxTotalDist, maxDistOutdoors)

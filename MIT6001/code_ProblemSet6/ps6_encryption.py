...
def applyCoder(text, coder):
    ...
    ### TODO.
    ciphertext=''
    noncipher = ' ' + string.punctuation + string.digits
    for w in text:
        if w not in noncipher:
            ciphertext += coder[w]
        else:
            ciphertext += w
    return ciphertext
...
def findBestShift(wordList, text):
    ...
    for shift in range(0,26):
        ...
        plain_text_words = plain_text.split(' ' and ',' and '!' and '.')
        real_words = 0
        for w in plain_text_words:
            if w in wordList:
                real_words += 1
        if max_real_words< real_words:
